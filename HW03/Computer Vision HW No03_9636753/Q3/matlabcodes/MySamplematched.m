...
function counterResult = MySamplematched (I) 
 ...
 %X=im2double(imread('D:\IUT\ComputerVision\HW\HW03\Q3\Dataset\Test_68_38.tif'));
 %I=medfilt2(X,[3 3]);
 %I=im2double(I);
    counterResult=0;
    % Large Sample 54*72
    ...
    for i = 1:8
        path = strcat('D:\IUT\ComputerVision\HW\HW03\Q3\pattern\3\',num2str(i),'.tif');
        L_templates(i,:,:) = im2double(imread(path));
        path = strcat('D:\IUT\ComputerVision\HW\HW03\Q3\pattern\2\',num2str(i),'.tif');
        M_templates(i,:,:) = im2double(imread(path));
        path = strcat('D:\IUT\ComputerVision\HW\HW03\Q3\pattern\1\',num2str(i),'.tif');
